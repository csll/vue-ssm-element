<template>
  ...
  <el-table
    ...
    <el-table-column
      ...
      width="120">
      <template slot-scope="scope">
        <el-switch v-model="scope.row.mgState" @click="userChange(scope.row)" @change="userStateChanged(scope.row)"></el-switch>
      </template>
    </el-table-column>
    <el-table-column
      prop="mobile"
  ...
  methods: {
    ...
    userStateChanged (userInfo) {
      if (userInfo.roleName === '超级管理员') {
        this.$message.error('修改超级管理员信息失败')
        this.getUserList()
        return
      }
    ...
    changeRole (user) {
      ...
      this.axios({
        method: 'POST',
        url: 'http://127.0.0.1:8008/api/private/v1/users/updateRole',
        data: {
          id: user.id,
        ...
        if (res.data.meta.status === 200) {
          vm.$message.success('用户身份修改成功')
          vm.getUserList()
        } else {
          vm.$message.warning('不能修改超级管理员身份失败')
        }
      })
    },
